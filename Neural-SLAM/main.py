...
def main():
    ...
    for ep_num in range(num_episodes):
        for step in range(args.max_episode_length):
            ...
            #    torch.set_grad_enabled(False)
            #l_action = action.cpu()
            l_action = output[:, -1].long().cpu()
            #print(action.cpu(),output[:, -1],l_action)
            # ------------------------------------------------------------------
            ...
            for env_idx in range(num_scenes):
                full_map[env_idx,0,:,:] = torch.from_numpy(np.asarray(
                    infos[env_idx]['map']
                )).float().to("cpu")
                full_map[env_idx, 1, :, :] = torch.from_numpy(np.asarray(
                    infos[env_idx]['explored_map']
                )).float().to("cpu")

                loc_r,loc_c = infos[env_idx]['rc']
                rc.append((loc_r,loc_c))
                full_map[env_idx, 2:, loc_r - 2:loc_r + 3, loc_c - 2:loc_c + 3] = 1.
            ...
            if (total_num_steps * num_scenes) % args.save_interval < \
                ...
                if len(g_episode_rewards) >= 10 and \
