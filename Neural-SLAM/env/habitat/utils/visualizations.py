...
def get_colored_map_semantic(mat, collision_map, semantic_map, semantic_palette, visited, visited_gt, goal,
                    explored, gt_map, gt_map_explored):
    m, n = mat.shape
    colored = np.zeros((m, n, 3))
    pal = sns.color_palette("Paired")
    ...
    for i in range(len(semantic_map)):
       colored = fill_color(colored, semantic_map[i], semantic_palette[i])
